...
class PyramidPoolingModule_rnn(nn.Module):
    def __init__(self,inputsize=2048, pyramids=[1,2,3,6]):
        super(PyramidPoolingModule_rnn, self).__init__()
        self.pyramids = pyramids
        self.hiddensize = inputsize // 2
    ...
    def forward(self, x):
        ...
        for bin_size in self.pyramids:
            feat_x = F.adaptive_avg_pool2d(x, output_size=bin_size)
            if bin_size == 0 :
                h0=torch.zeros(2,1,self.hiddensize).to("cuda")
                feat_x,hn=self.RNN(feat_x.reshape(feat_x.size(0),feat_x.size(1)),h0)
                feat_x.permute(0,2,1).unsqueeze(3)
            feat_x = F.interpolate(feat_x, size=(height, width), mode='bilinear', align_corners=True)
            feat  = feat + feat_x
        return feat
...
class ICNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.conv_sub1 = nn.Sequential(
            Conv3x3BNReLU(1, 32, 2),
            Conv3x3BNReLU(32, 32, 2),
            Conv3x3BNReLU(32, 64, 2)
        )
        self.backbone = Backbone()
        self.ppm = PyramidPoolingModule()
        # self.ppm_rnn = PyramidPoolingModule_rnn()
